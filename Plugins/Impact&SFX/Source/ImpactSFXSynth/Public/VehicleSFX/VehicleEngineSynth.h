...
namespace LBSImpactSFXSynth
{
	...
	struct FVehicleEngineParams
	{
		...
		FVehicleEngineParams(const float InRPM,
							...
							const float InF0Fluctuation,
							const float InHarmonicFluctuation,
							const float InHarmonicGain,
							const float InAmpRandMin,
							const float InAmpRandMax,
							const int32 InNumModalsDeceleration,
							const float InFreqScale,
							const float InCutoffFreq,
							const float InFallOffDb)
		: RPMNoiseFactor(InRPMNoiseFactor),
		  ...
		  FallOffDb(InFallOffDb)
		{
			HarmonicFluctuation = FMath::Max(0.f, InHarmonicFluctuation);
			HarmonicGain = FMath::Max(0.f, InHarmonicGain);
			IdleRPM = FMath::Max(1.f, InIdleRPM + 1.f); //Plus 1 for a small offset
			CutoffFreq = FMath::Clamp(InCutoffFreq, 20.f, 20e3f);
			RPM = FMath::Max(InRPM, 0.f);
		}
	};
