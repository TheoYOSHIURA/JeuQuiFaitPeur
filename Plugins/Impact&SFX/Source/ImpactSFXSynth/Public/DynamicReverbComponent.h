...
protected:
	...
	/** If the squared distance between the current location and the last trace location is higher than this value, internal states are reset.	*/
	UPROPERTY(EditDefaultsOnly, Category = "Tracing")
	float ResetDistance;
	...
	/** If true, print logs about rare occurrences in the logic flow (skip traces, reset stats, etc.). Only used in Editor.*/
    UPROPERTY(EditDefaultsOnly, Category = "Tracing Debug")
    bool bLogOperationFlow;
	
public:
	UDynamicReverbComponent();

	virtual void GetRoomStats(const FVector& AudioLocation,
							   float& OutRoomSize, float& OutOpenFactor, float& OutAbsorption, float& OutEnclosedFactor) const;
