	...
	public:
		...
		void CreateModalObj()
		{
			...
			for(int i = 0, j = 0; j < NumTrueModal; i += FModalSynth::NumParamsPerModal, j++)
			{
				...
				const float NewDecay = FMath::Max(OutputModalData[DecayIdx] * (1.0f + FRandBipolar() * DecayRandRange), FModalSynth::DecayMin);
		...
		void Randomize(const FImpactModalObjAssetProxyPtr& ModalPtr, const float NumTrueModals)
		{
			...
			for(int i = 0, j = 0; j < NumTrueModals; i += FModalSynth::NumParamsPerModal, j++)
			{
				...
				const int32 DecayIdx = i + FModalSynth::DecayBin;
				const float Decay = FMath::Max(ModalData[DecayIdx] * (1.0f + DecayRandRange * FRandBipolar()), FModalSynth::DecayMin);
				DecayBuffer[j] = Decay;
		...
		FORCEINLINE float FRandBipolar() const
		{
